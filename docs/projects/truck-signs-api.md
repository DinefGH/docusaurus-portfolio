...
Welcome to Signs for Trucks, an online Django-based API and administration backend for managing and selling vinyl truck signage. This project provides RESTful endpoints for categories, products, orders, payments, comments, and customer image uploads, complete with a Dockerized deployment using PostgreSQL and Gunicorn.
...
Docker & Docker CLI (to build and run containers)

## Quickstart

To get up and running in minutes, use the Docker workflow described below. All containers (API and database) are configured to communicate over a Docker network and share environment settings via a root-level .env file.

1. Clone GitHub Repository

    ```bash
    git clone https://github.com/DinefGH/truck_signs_api.git
    cd truck_signs_api
    ```

2. Environment Configuration

    We’ve provided a **simple_env_config.env** file with dummy (unsafe) values so you can run the app immediately:

    2.1 **Copy** it to `.env` in the repo root:

    ```bash
    cp simple_env_config.env .env
    ```

    2.2 (Optional) Edit .env and replace any values with your own secrets before deploying to production.

3. Build Docker Image

    Build the Django API container image using the provided Dockerfile:
...
4. Create a Docker network
    ...
    This network allows the API and database containers to communicate

    ```bash
    docker network create truck-net
    ```
...
6. Start the Django API container
...
7. Verify the deployment
    ...
    # View logs for the API container to ensure no errors
    docker logs -f truck-api
    ```

8. Access the API

    Open your browser or use curl to hit the products endpoint:

    curl -i http://YOUR_VM_IP:8020/truck-signs/products/

    You should receive a 200 OK and a JSON array (empty if no products exist yet).

    Now your API is up and running at http://YOUR_VM_IP:8020/.
...
2. Creating a Superuser
    ...
    2.1 Open a shell in the Django container

    ```bash
    # Open a shell inside the running container
    docker exec -it truck-api sh
    ```

    2.2 Run the createsuperuser command:
    ...
    2.4 Log in to the admin panel:

    Visit http://YOUR_VM_IP:8020/admin/ and sign in with the credentials you just created.
...
4. Stop and Restart
    ...
    ```bash
    # Stop
    docker stop truck-api

    # Start again
    docker start truck-api
    ```
